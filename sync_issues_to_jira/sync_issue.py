...
def handle_issue_closed(jira, event):
    ...
    print("event[issue][pull_request]: " + str(event["issue"]["pull_request"]))
...
def handle_pr_closed(jira, event):
    ...
    gh_issue_number = event["number"]
    print("repo: " + str(repo))
    print("issue: " + str(repo.get_issue(gh_issue_number)))
    events = repo.get_issue(gh_issue_number).get_events()
    ...
    if closed_with_commit:
        print()
        repo.get_issue(gh_issue_number).create_comment("This comment was cherry-picked")
